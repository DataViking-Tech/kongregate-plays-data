...
def cdx_url(original: str, collapse: str = "digest", from_timestamp: str = "", to_timestamp: str = "") -> str:
    params = [
        ("url", original),
        ("output", "json"),
        ("fl", ",".join(FIELDS)),
...
def cdx_cache_path(seed_id: str, collapse: str, from_timestamp: str, to_timestamp: str) -> Path:
    if collapse == "digest" and not from_timestamp and not to_timestamp:
        return RAW_CDX / f"{seed_id}.json"
    collapse_label = collapse or "none"
    suffix = safe_name(f"collapse_{collapse_label}_from_{from_timestamp or 'start'}_to_{to_timestamp or 'end'}")
    return RAW_CDX / f"{seed_id}_{suffix}.json"
...
def fetch_cdx(
    ...
    cache = cdx_cache_path(seed_id, collapse, from_timestamp, to_timestamp)
    if cache.exists() and not refresh:
        return json.loads(cache.read_text())
    request = urllib.request.Request(
        cdx_url(original, collapse=collapse, from_timestamp=from_timestamp, to_timestamp=to_timestamp),
...
def main() -> None:
    ...
    parser.add_argument("--collapse", default="digest", help="CDX collapse mode. Use an empty string to retain every capture.")
    ...
    report = {
        ...
        "cdx_collapse": args.collapse or "none",
        "cdx_from_timestamp": cdx_from_timestamp,
        "cdx_to_timestamp": cdx_to_timestamp,
        "note": (
            "CDX inputs are digest-collapsed, so duplicate archived HTML content is intentionally not fetched repeatedly."
            if args.collapse
            else "CDX inputs were not collapsed, retaining every capture in the requested timestamp range."
        ),
    }
    ...
    REPORT_MD_PATH.write_text(
        "\n".join(
            [
                ...
                f"- CDX collapse: {report['cdx_collapse']}",
                f"- CDX timestamp range: {report['cdx_from_timestamp'] or 'start'} to {report['cdx_to_timestamp'] or 'end'}",
                "",
                report["note"],
                "",
            ]
