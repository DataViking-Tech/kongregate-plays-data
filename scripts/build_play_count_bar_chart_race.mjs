...
const sheetUrl = "https://docs.google.com/spreadsheets/d/1NeN3WR_qNommOPW0wphly-PWhOBiYZvDnZ7LFb9fh7Q";
  ...
  <script>
    ...
    function syncMotionTiming() {
      ...
      const duration = playbackMode === "smooth"
        ? Math.max(48, Math.min(95, delay * 0.62))
    ...
    function schedule() {
      ...
      function tick(now) {
        ...
        if (now - lastTick >= delay) {
          const steps = Math.min(3, Math.max(1, Math.floor((now - lastTick) / delay)));
