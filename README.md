...
https://docs.google.com/spreadsheets/d/1Ys_eyJLfc_QRR2siXMi6LJMxfLbzL_pAql_mmhHe58o
...
- Per-game metrics history rows: 5,020 across 1,286 canonical games
- Observed play-count rows used by the chart: 16,107
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,800 observed capture-date frames.
...
- 1,308 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 738 missing CDX cache files; remaining unresolved games include 617 no-CDX cases, 647 CDX-cache-missing cases, and 44 games with known failures only.
- 0 true play-count decreases remain after separating 121 stale listing-page echoes into `stale_listing_play_counts.csv`.
