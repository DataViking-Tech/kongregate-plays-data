...
https://docs.google.com/spreadsheets/d/1_7aLEM5RePmxVHy0wFP8JSjHmHTrnrn_1pa4H9kU4u0
...
- Per-game metrics history rows: 7,250 across 2,569 canonical games
- Observed play-count rows used by the chart: 21,768
- Chart playback: Smooth mode uses 7,457 interpolated month-paced display frames by default; Captures mode exposes all 2,138 observed capture-date frames.
...
- Metrics gap audit currently has 0 fresh pending captures, 226 known failed archived captures, 367 unresolved no-CDX cases, 71 missing CDX cache files noted for targeted follow-up, and 0 known-failures-only cases.
- 6 source-conflict play-count decreases are under review after separating 220 stale listing-page echoes into `stale_listing_play_counts.csv`.
