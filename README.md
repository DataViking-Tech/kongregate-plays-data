...
https://docs.google.com/spreadsheets/d/1SzPejPyuW6oDAsSX4furt7Y4ZnjZVTTHyRSvAJ4gACU
...
- Per-game metrics history rows: 5,061 across 1,289 canonical games
- Observed play-count rows used by the chart: 16,148
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,803 observed capture-date frames.
...
- 1,305 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 683 missing CDX cache files; remaining unresolved games include 653 no-CDX cases, 599 CDX-cache-missing cases, and 53 games with known failures only.
