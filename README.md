...
- Per-game metrics history rows: 5,403 across 1,318 canonical games
- Observed play-count rows used by the chart: 16,490
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,852 observed capture-date frames.
...
- 1,270 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 583 missing CDX cache files; remaining unresolved games include 736 no-CDX cases, 502 CDX-cache-missing cases, and 32 games with known failures only.
