...
https://docs.google.com/spreadsheets/d/1gL1LroY5Ji0IhOWDHR2KSnaklAjKMYxqJDgz_gzKwB4
...
- Per-game metrics history rows: 5,273 across 1,309 canonical games
- Observed play-count rows used by the chart: 16,360
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,831 observed capture-date frames.
...
- 1,280 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 635 missing CDX cache files; remaining unresolved games include 694 no-CDX cases, 553 CDX-cache-missing cases, and 33 games with known failures only.
