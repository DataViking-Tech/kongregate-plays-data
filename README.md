...
https://docs.google.com/spreadsheets/d/1bmJMjRCeXUU7Gzx4E-uhxwxH9T396wg7ULugmppLNkg
...
- Per-game metrics history rows: 5,194 across 1,308 canonical games
- Observed play-count rows used by the chart: 16,281
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,827 observed capture-date frames.
...
- 1,282 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 653 missing CDX cache files; remaining unresolved games include 678 no-CDX cases, 570 CDX-cache-missing cases, and 34 games with known failures only.
