...
https://docs.google.com/spreadsheets/d/1rsVWnlFzRwE3NyZP89eBHBKKaVNRgdhGBQcXxZ8ME38
...
- Per-game metrics history rows: 5,084 across 1,291 canonical games
- Observed play-count rows used by the chart: 16,171
- Chart playback: Smooth mode uses 2,331 interpolated display frames by default; Captures mode exposes all 1,806 observed capture-date frames.
...
- 1,303 mini-catalog games still have no per-game metrics rows, and 2,311 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 675 missing CDX cache files; remaining unresolved games include 660 no-CDX cases, 592 CDX-cache-missing cases, and 51 games with known failures only.
