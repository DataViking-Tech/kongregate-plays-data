...
https://docs.google.com/spreadsheets/d/1-lADh7WlnBtAXZC6gmUI9sEWm4ztiPwVuiYc8R5nw2g

## Current Snapshot

- Ranked-list rows: 39,003
- Ranked-list rows with observed play counts: 12,061
- Mini catalog: 3,129 games that reached top 20 in observed rankings
- Per-game metrics history rows: 6,500 across 2,299 canonical games
- Observed play-count rows used by the chart: 18,561
- Chart playback: Smooth mode uses buffered, interpolated display frames by default; Captures mode exposes all 1,978 observed capture-date frames.
...
- Ranked/frame scrape backlog now has 0 pending captures in the latest processed slices, alongside 2,655 cached manifest entries, 7,641 ranked-page failures, and 696 frame failures.
- 39 historical months still have no ranked-list captures in the processed dataset.
- 357 mini-catalog games still have no per-game metrics rows, and 2,376 still need deeper page-history backfill.
- Metrics gap audit currently has 0 fresh pending captures and 382 missing CDX cache files; remaining unresolved games include 347 no-CDX cases, 0 CDX-cache-missing cases, and 10 games with known failures only.
